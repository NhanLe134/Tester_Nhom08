...
function seed() {
  ...
  if (existing.cnt > 0) { console.log('Database already seeded.'); return; }
  ...
  transaction(db, () => {
    ...
    const insP = db.prepare(`INSERT INTO HANGHOA (MASP,TENSP,DVT,GIABAN,GIANHAP,SL_TON,DMUC_TON_MIN,TRANGTHAI_SP) VALUES (?,?,?,?,?,?,?,'Đang bán')`);
    for (const p of products) insP.run(p.MASP,p.TENSP,p.DVT,p.GIABAN,p.GIANHAP,p.SL_TON,p.DMUC_TON_MIN);

    const insH = db.prepare(`INSERT INTO HOADONBAN (MAHDB,NGUOITAO,TONGTIENHANG_BAN,NGAYBAN,PTTT,TRANGTHAI_HDB,GHICHU) VALUES (?,?,?,?,?,?,NULL)`);
    const insC = db.prepare(`INSERT INTO CT_HOADONBAN (MAHDB,MASP,SOLUONG,GIABAN) VALUES (?,?,?,?)`);
    ...
    // 80% hoàn thành, 20% hủy
    const statusList = ['Hoàn thành','Hoàn thành','Hoàn thành','Hoàn thành','Đã hủy'];

    let hdbNum = 1;

    // Sinh hóa đơn cho toàn bộ năm 2026: 1/1 -> 31/12/2026
    // Mỗi ngày 5-12 hóa đơn để có nhiều dữ liệu
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      for (let k = 0; k < numInvoices; k++) {
        ...
        insH.run(mahdb, adminId, total, dateTime, pttt, status);
        for (const it of items) insC.run(mahdb, it.MASP, it.SL, it.G);
      }
    }
